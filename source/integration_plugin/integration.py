...
class Integration(Process):
    ...
    def __init__(self, cwd, name, path, filename, arguments, format, package):
        super(Integration, self).__init__(cwd, name)
        self.duration = 0
    ...
    def report_properties(self):
        ...
        properties.append(('Package', self.package.format_name()))
        properties.append(('Duration', str(self.duration) + ' seconds'))
        return properties
        
    def _copy_resources(self):
        self.package.unpack(self.cwd, Step.WORKSPACE, flatten=True)
